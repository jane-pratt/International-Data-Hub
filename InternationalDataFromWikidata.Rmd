...
We can retrieve all instances of a city, along with their country and coordinates. 

```{r}
cities <- query_wikidata(
'SELECT ?city ?cityLabel ?coordinate_location ?country ?countryLabel WHERE {
  SERVICE wikibase:label { bd:serviceParam wikibase:language "[AUTO_LANGUAGE],en". }
  ?city wdt:P31 wd:Q515.
  OPTIONAL { ?city wdt:P625 ?coordinate_location. }
...
## Getting coordinates for the capital city (incomplete - need to edit Wikidata)
...
wikidata_capital_coords %>% arrange(capitalLabel)
```
...
Unfortunately at this point there are some capital cities with no coordinates. Further investigation shows that some of the cities do not have the Instance of ... Capital property set. I've added these for some cities but others are locked. The docs suggest that a user is able to edit these after 4 days, so we will put this code on hold for the moment.
```{r}
...
Tutorial on using the Wikidata Query Service: 
https://wdqs-tutorial.toolforge.org/

Documentation for WikidataQueryServiceR package:
https://www.rdocumentation.org/packages/WikidataQueryServiceR/versions/1.0.0

ISO 3166-1 Codes:
https://en.wikipedia.org/wiki/ISO_3166-1

Blog Post 'Why should you learn SPARQL? Wikidata!'
